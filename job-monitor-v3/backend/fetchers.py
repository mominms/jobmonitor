...
def save_lead(lead_data):
    ...
    try:
        ...
        with get_read_connection() as conn:
            exists = conn.execute("SELECT id FROM job_leads WHERE external_id = ?", (lead_data['external_id'],)).fetchone()
            if exists:
                return False
        ...
        with get_write_connection() as conn:
            # Double check inside lock just in case? Not strictly needed if ID is unique enough,
            # but safer for 'first writer wins' if parallel same job.
            # We'll rely on UNIQUE constraint handling or just proceed.
            conn.execute("""
...
def run_all_fetchers():
    ...
    with get_read_connection() as conn:
        sources = conn.execute("SELECT * FROM job_sources WHERE enabled = 1").fetchall()
